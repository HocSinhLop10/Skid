...
local function teleportToFarthestFromKillers()
    ...
    for _, gen in ipairs(mapFolder:GetChildren()) do
        if gen.Name == "Generator" and gen:FindFirstChild("Progress") then
            ...
            local distToNearestKiller = nearestKillerDistance(genPos)

            if distToNearestKiller > bestScore then
                bestScore = distToNearestKiller
                bestGen = gen
            end
        end
    end
    ...
    if bestGen then
        ...
        print(("✅ Teleported to generator xa killers nhất: %s (%.1f studs)")
            :format(bestGen.Name, bestScore))
    else
